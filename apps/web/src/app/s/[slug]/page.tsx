...
// time fetch from the public, field-whitelisted Elysia /api/store/:slug. Delegates
// to the REGISTRY-resolved template Listing component (workstream B).
...
export default async function StorefrontPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const data = await fetchStore(slug);
  if (!data) notFound();
  const { Listing } = getTemplate(data.config.templateId);
  return <Listing config={data.config} products={data.products} slug={slug} />;
}
