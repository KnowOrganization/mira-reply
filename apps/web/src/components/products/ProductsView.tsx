...
// public /s/<slug> site. The customizer previews the SAME shared Storefront tree
// the public page renders (preview == production), fed by draft settings.
import { useState } from "react";
import { Plus, ShoppingBag, Trash2, X, Pencil, ExternalLink, AlertTriangle, Star, Smartphone, Monitor } from "lucide-react";
import {
...
import { Storefront, type SfProduct } from "../../app/s/_components/Storefront";
...
export function ProductsView() {
  const [tab, setTab] = useState<"products" | "storefront">("products");
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0" style={{ background: "var(--bg-frame)" }}>
      <div className="px-6 pt-5 pb-2 flex items-center gap-3 shrink-0">
        ...
        <Segmented
          options={[{ id: "products", label: "Products" }, { id: "storefront", label: "Storefront" }]}
          value={tab}
          onChange={(t) => setTab(t)}
        />
      </div>
      <PublishBar s={s} />
      {tab === "products" ? <ProductsTab featuredIds={s.storefrontFeaturedIds ?? []} /> : <StorefrontTab settings={s} />}
    </div>
  );
}

// ── publish bar (slug + publish + view) ──────────────────────────────────────
function PublishBar({ s }: { s: StorefrontSettings }) {
  ...
  return (
    <div className="mx-6 mb-3 px-4 py-2.5 rounded-xl flex flex-wrap items-center gap-3" style={{ background: "var(--bg-elev)", border: "1px solid var(--border)" }}>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-6 pb-3 flex items-center gap-2.5 shrink-0">
        ...
        <button onClick={() => setEditing(toDraft())} className="ml-auto flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12.5px] font-semibold"
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          <div className="grid gap-3.5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))" }}>
            {list.isLoading && products.length === 0
              ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="rounded-xl animate-pulse" style={{ background: "var(--bg-inset)", aspectRatio: "4 / 5" }} />)
              : products.map((p) => (
        ...
        {editing && (
          <ProductEditor draft={editing}
            ...
            onSave={async (d) => {
              ...
              const body = {
                title: (d.title ?? "").trim(), subtitle: (d.subtitle ?? "").trim(), description: (d.description ?? "").trim(),
                priceText: (d.priceText ?? "").trim() || null, imageUrl: (d.imageUrl ?? "").trim() || null, ctaUrl: (d.ctaUrl ?? "").trim() || null,
                available: d.available ?? true, aliases,
              };
              try { if (d.id) await update.mutateAsync({ id: d.id, ...body }); else await create.mutateAsync(body); setEditing(null); } catch { /* surfaced */ }
            }} />
        )}
      </Modal>
    </div>
...
function AdminProductCard({ p, featured, onToggleFeatured, onEdit, onToggleAvailable, onDelete }: {
  p: Product; featured: boolean; onToggleFeatured: () => void; onEdit: () => void; onToggleAvailable: () => void; onDelete: () => void;
}) {
  return (
    <div className="group rounded-xl overflow-hidden flex flex-col transition-all duration-100"
      ...
      <div className="p-2.5 flex flex-col gap-1 flex-1">
        ...
        <button onClick={onToggleAvailable} className="text-[10px] font-medium px-1.5 py-0.5 rounded-md self-start" style={p.available ? { background: "var(--accent-soft)", color: "var(--accent-deep)" } : { background: "var(--bg-inset)", color: "var(--text-subtle)" }}>
...
function StorefrontTab({ settings }: { settings: StorefrontSettings }) {
  ...
  const patch = usePatchIgSettings();
  const products = list.data?.products ?? [];
  // local overlay for instant preview; text commits on blur, the rest immediately.
  ...
  const commit = (k: keyof StorefrontSettings) => { const v = (local as any)[k]; if (v !== undefined && v !== (settings as any)[k]) patch.mutate({ [k]: v }); };
  const setNow = (obj: Partial<StorefrontSettings>) => { setLocal((l) => ({ ...l, ...obj })); patch.mutate(obj); };

  const config = resolveStorefrontConfig(eff as StorefrontSettingsInput, products.map((p) => ({ id: p.id, available: p.available, imageUrl: p.imageUrl })));
  ...
  return (
    <div className="flex-1 min-h-0 flex" style={{ borderTop: "1px solid var(--border)" }}>
      ...
      <div className="w-[360px] shrink-0 overflow-y-auto p-5 flex flex-col gap-5" style={{ borderRight: "1px solid var(--border)" }}>
        <Group label="Hero">
          <Field label="Headline"><input value={eff.storefrontHeroHeadline ?? ""} onChange={(e) => setText("storefrontHeroHeadline", e.target.value)} onBlur={() => commit("storefrontHeroHeadline")} placeholder={config.title} style={inputStyle} /></Field>
          <Field label="Tagline"><input value={eff.storefrontHeroTagline ?? ""} onChange={(e) => setText("storefrontHeroTagline", e.target.value)} onBlur={() => commit("storefrontHeroTagline")} placeholder="One line about your shop" style={inputStyle} /></Field>
          <Field label="Hero image URL" hint="Public https. Falls back to a product image."><input value={eff.storefrontHeroImageUrl ?? ""} onChange={(e) => setText("storefrontHeroImageUrl", e.target.value)} onBlur={() => commit("storefrontHeroImageUrl")} placeholder="https://…" style={inputStyle} /></Field>
          <Field label="Layout"><Segmented size="sm" options={[{ id: "split", label: "Split" }, { id: "minimal", label: "Minimal" }]} value={(eff.storefrontHeroLayout as string) || "split"} onChange={(v) => setNow({ storefrontHeroLayout: v as "split" | "minimal" })} /></Field>
        </Group>
          ...
          <div className="flex flex-wrap gap-1.5">
            {PRESET_ACCENTS.map((c) => (
              <button key={c} onClick={() => setNow({ storefrontAccent: c })} title={c}
                className="w-6 h-6 rounded-md" style={{ background: c, outline: (eff.storefrontAccent || "#4f6bed") === c ? "2px solid var(--text)" : "1px solid var(--border)", outlineOffset: 1 }} />
            ))}
            <input type="color" value={config.accent} onChange={(e) => setNow({ storefrontAccent: e.target.value })} className="w-6 h-6 rounded-md cursor-pointer" style={{ border: "1px solid var(--border)", background: "transparent" }} />
          </div>
        </Group>

        <Group label="Buy button">
          <Segmented size="sm" options={[{ id: "Buy", label: "Buy" }, { id: "Shop", label: "Shop" }, { id: "Order", label: "Order" }]} value={(eff.storefrontBuyLabel as string) || "Buy"} onChange={(v) => setNow({ storefrontBuyLabel: v as "Buy" | "Shop" | "Order" })} />
        </Group>
        ...
        <Group label="Sections">
          ...
          {eff.storefrontShowAbout && (
            <textarea value={eff.storefrontAbout ?? ""} onChange={(e) => setText("storefrontAbout", e.target.value)} onBlur={() => commit("storefrontAbout")} rows={3} placeholder="Tell your story…" style={{ ...inputStyle, resize: "none", lineHeight: 1.5 }} />
          )}
          {eff.storefrontShowAbout && <Field label="Contact URL"><input value={eff.storefrontContactUrl ?? ""} onChange={(e) => setText("storefrontContactUrl", e.target.value)} onBlur={() => commit("storefrontContactUrl")} placeholder="https://…" style={inputStyle} /></Field>}
        </Group>
        ...
        <Group label={`Featured (${featuredIds.length})`} hint="Star products in the Products tab to feature them.">
          <div className="flex flex-col gap-1">
            {featuredIds.map((id) => {
              ...
              if (!p) return null;
              return (
                <div key={id} className="flex items-center gap-2 text-[12px] px-2 py-1 rounded-md" style={{ background: "var(--bg-inset)" }}>
                  <Star size={11} style={{ color: "#f5a623" }} fill="#f5a623" />
                  <span className="truncate flex-1">{p.title}</span>
                  <button onClick={() => setNow({ storefrontFeaturedIds: featuredIds.filter((x) => x !== id) })} style={{ color: "var(--text-subtle)" }}><X size={12} /></button>
                </div>
              );
            })}
            {featuredIds.length === 0 && <span className="text-[11px]" style={{ color: "var(--text-subtle)" }}>None — showing first products.</span>}
          </div>
        </Group>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col" style={{ background: "var(--bg-inset)" }}>
        <div className="h-10 shrink-0 flex items-center justify-center gap-2" style={{ borderBottom: "1px solid var(--border)" }}>
          <Segmented size="sm" options={[{ id: "phone", label: "Phone" }, { id: "desktop", label: "Desktop" }]} value={device} onChange={(d) => setDevice(d)} />
        ...
        <div className="flex-1 overflow-y-auto flex justify-center p-5">
          <div style={{ width: device === "phone" ? 390 : "100%", maxWidth: device === "phone" ? 390 : 1000, border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden", background: "var(--bg)", alignSelf: "start", boxShadow: "var(--shadow-card)" }}>
            <Storefront config={config} products={sfProducts} />
          </div>
        </div>
      </div>
    </div>
...
function Toggle({ label, on, onChange }: { label: string; on: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!on)} className="flex items-center justify-between text-[12.5px]" style={{ color: "var(--text)" }}>
      <span>{label}</span>
      <span style={{ width: 34, height: 20, borderRadius: 999, background: on ? "var(--accent)" : "var(--bg-inset)", border: "1px solid var(--border)", position: "relative", transition: "background .15s" }}>
        <span style={{ position: "absolute", top: 1.5, left: on ? 15 : 1.5, width: 16, height: 16, borderRadius: 999, background: "#fff", transition: "left .15s", boxShadow: "0 1px 2px rgba(0,0,0,0.2)" }} />
      </span>
    </button>
  );
}
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 flex flex-col gap-3">
            <Field label="Title"><input autoFocus value={d.title ?? ""} onChange={(e) => set({ title: e.target.value })} placeholder="Oversized Cotton Hoodie" style={inputStyle} /></Field>
        ...
        <Field label="Image URL" hint={badImg ? "⚠ Must be an https URL." : "Public https link. Avoid Instagram URLs (they expire)."}>
          <input value={d.imageUrl ?? ""} onChange={(e) => set({ imageUrl: e.target.value })} placeholder="https://…/photo.jpg" style={{ ...inputStyle, borderColor: badImg ? "#ef4444" : "var(--border)" }} />
        </Field>
        <Field label="Buy / link-out URL" hint="Where Buy sends customers (your checkout, WhatsApp, site)."><input value={d.ctaUrl ?? ""} onChange={(e) => set({ ctaUrl: e.target.value })} placeholder="https://…" style={inputStyle} /></Field>
        <Field label="Aliases" hint="Comma-separated synonyms so Mira matches casual asks (hoodie, pullover)."><input value={d.aliasesText ?? ""} onChange={(e) => set({ aliasesText: e.target.value })} placeholder="hoodie, pullover" style={inputStyle} /></Field>
        <Field label="Description" hint="Long copy for the product page."><textarea value={d.description ?? ""} onChange={(e) => set({ description: e.target.value })} rows={3} placeholder="Soft brushed cotton…" style={{ ...inputStyle, resize: "none", lineHeight: 1.5 }} /></Field>
      ...
      <div className="px-5 h-14 flex items-center justify-end gap-2 border-t shrink-0" style={{ borderColor: "var(--border)" }}>
        ...
        <button onClick={() => canSave && onSave(d)} disabled={!canSave || saving} className="h-8 px-4 rounded-lg text-[12.5px] font-semibold transition-opacity" style={{ background: "var(--accent)", color: "var(--accent-fg)", opacity: canSave && !saving ? 1 : 0.5 }}>
