// Analytics / brain read routes — ports of the Next app/api/ig/* handlers.
// Every handler is gated by a valid BetterAuth session (requireUser); the
// store is always read per-account via readStore(accountId).
...
const TOPICS: FactTopic[] = ["personal", "gear", "location", "song", "shop", "general"];
const asTopic = (t: unknown): FactTopic =>
...
export const analyticsRoute = new Elysia()
  // GET /api/ig/dashboard — full account analytics snapshot for the UI.
  .get("/api/ig/dashboard", async ({ request, set }) => {
  ...
  // GET /api/ig/feed — raw feed events for the account.
  .get("/api/ig/feed", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
  ...
  // GET /api/ig/logs — recent message-send logs (?limit, capped at 1000).
  .get("/api/ig/logs", async ({ request, query, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
  ...
  // GET /api/ig/digest — last-24h activity summary.
  .get("/api/ig/digest", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const s = await readStore(a.ctx.accountId);
  ...
  // GET /api/ig/brain — account-scoped facts + per-topic stats + gaps.
  .get("/api/ig/brain", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const s = await readStore(a.ctx.accountId);
  ...
  // POST /api/ig/brain — add a fact, extract facts from free text, or delete a fact.
  .post("/api/ig/brain", async ({ request, body, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const b = (body ?? {}) as {
    ...
    if (b.action === "delete") {
      if (!b.id) { set.status = 400; return { error: "id required" }; }
    ...
    if (b.action === "add") {
      ...
      if (!question || !answer) { set.status = 400; return { error: "question + answer required" }; }
  ...
  // GET /api/ig/brain-stats — brain MCP tool stats + available tools.
  .get("/api/ig/brain-stats", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    return {
  ...
  // GET /api/ig/brain-probe — smoke test for the brain MCP; runs each tool, reports timings.
  .get("/api/ig/brain-probe", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
  ...
  // POST /api/ig/cleanup — purge in-app test data (injected comments, drafts, facts, fake users).
  .post("/api/ig/cleanup", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
