// IG posts + per-post actions (comments / insights / extract / reply-all),
// scoped to the logged-in user's account. Ported 1:1 from the Next routes.
...
export const postsRoute = new Elysia()
  // GET /api/ig/posts — list posts newest-first
  .get("/api/ig/posts", async ({ request, set }) => {
  ...
  // POST /api/ig/posts — sync (same behavior as POST /api/ig/posts/sync)
  .post("/api/ig/posts", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
  ...
  // POST /api/ig/posts/sync — paginated full media sync
  .post("/api/ig/posts/sync", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    return syncPosts(a.ctx.accountId, set);
  ...
  // GET /api/ig/posts/:id — single post
  .get("/api/ig/posts/:id", async ({ request, params, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const s = await readStore(a.ctx.accountId);
    const p = s.posts[params.id];
    if (!p) { set.status = 404; return { error: "not found" }; }
    return { post: p };
  })

  // PATCH /api/ig/posts/:id — notes / Q&A / links
  .patch("/api/ig/posts/:id", async ({ request, params, body, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
  ...
  // GET /api/ig/posts/:id/comments — live-fetch + merge into shared cache
  .get("/api/ig/posts/:id/comments", async ({ request, params, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const accountId = a.ctx.accountId;
  ...
  // GET /api/ig/posts/:id/insights — fetch + persist post insights
  .get("/api/ig/posts/:id/insights", async ({ request, params, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const accountId = a.ctx.accountId;
  ...
  // POST /api/ig/posts/:id/extract — LLM-extract notes/links/qa from a paragraph
  .post("/api/ig/posts/:id/extract", async ({ request, params, body, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const accountId = a.ctx.accountId;
  ...
  // POST /api/ig/posts/:id/reply-all — bulk classify + DM + comment-reply
  .post("/api/ig/posts/:id/reply-all", async ({ request, params, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
    const accountId = a.ctx.accountId;
  ...
  // POST /api/ig/posts/:id/reply-all/stop — raise the global stop flag
  .post("/api/ig/posts/:id/reply-all/stop", async ({ request, set }) => {
    const a = await requireUser(request.headers);
    if (!a.ctx) { set.status = a.status!; return { error: a.error }; }
    if (!a.ctx.accountId) { set.status = 404; return { error: "no account" }; }
...
// ── shared sync helper (posts POST + posts/sync POST) ────────────────────────
